class StudentGreeter
  ...
  def talk_page_blank?(student)
    WikiApi.new(wiki: student.home_wiki).get_page_content(student.talk_page).nil?
  end

  def a_greeter_already_posted?(student)
    contributor_ids = ids_of_contributors_to_talk_page(student)
    # TODO: Fix greeter wiki assumption
    return false if (@greeters.pluck(:native_id) & contributor_ids).empty?
  ...
  def ids_of_contributors_to_talk_page(user)
    contributors_response = WikiApi.new(wiki: user.home_wiki).query contributors_query(user.talk_page)
